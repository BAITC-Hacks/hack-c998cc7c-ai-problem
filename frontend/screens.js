import { state, t, esc, $, api, field, input } from './shared.js';
import { dateLabel, statusBadge, statusLabel, stageName } from './views.js';

export const pageHeading = (heading, subtitle, action = '') => `<div class="page-heading"><div><p class="eyebrow">${t('Жұмыс кеңістігі', 'Рабочее пространство')}</p><h1>${heading}</h1><p class="muted">${subtitle}</p></div>${action}</div>`;
const empty = (heading, body, action = '') => `<div class="empty"><span class="empty-symbol" aria-hidden="true">≋</span><h2>${heading}</h2><p>${body}</p>${action}</div>`;
...
export async function meetings() {
  ...
  $('#app').innerHTML = pageHeading(t('Кездесулер', 'Совещания'), t('Жазбалар, шешімдер және тапсырмалар — бір жерде.', 'Записи, решения и поручения вашей команды — в одном месте.'), `<a href="#upload" class="button primary">+ ${t('Жазбаны жүктеу', 'Загрузить запись')}</a>`) +
...
export async function settings() {
  ...
  const provider = (title, host, model, ready, variables) => `<section class="panel"><div class="section-heading"><h2>${title}</h2>${badge(ready)}</div><p>${esc(host || t('Провайдер таңдалмаған', 'Провайдер не выбран'))}</p><p>${t('Модель', 'Модель')}: <strong>${esc(model || '—')}</strong></p><p class="muted">${variables}</p></section>`;
  $('#app').innerHTML = pageHeading(t('Баптаулар', 'Настройки'), t('Қолданба жергілікті жұмыс істейді, AI — API арқылы.', 'Приложение работает локально, ИИ — через API.')) +
