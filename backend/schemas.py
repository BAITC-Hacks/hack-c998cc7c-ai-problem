...
class Assignment(Strict):
    ...
    unspecified: list[Literal['owner', 'deadline', 'expected_result']] = Field(default_factory=list)
...
def validate_evidence(analysis: Analysis, segments: list[Segment], require=True):
    ...
    for item in [*analysis.assignments, *analysis.decisions]:
        if require and not item.evidence and getattr(item, 'origin', 'extracted') == 'extracted':
        ...
        for e in item.evidence:
            ...
            if not s or e.quote not in s.text or e.start != s.start or e.end != s.end:
                raise ValueError('Evidence must match an existing segment and its exact timing')
