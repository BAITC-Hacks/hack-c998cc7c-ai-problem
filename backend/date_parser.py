...
WEEKDAYS = [(r'понедельн\w*|дүйсенбі\w*', 0), (r'вторн\w*|сейсенбі\w*', 1),
            (r'сред[ауы]|сәрсенбі\w*', 2), (r'четверг\w*|бейсенбі\w*', 3),
            (r'пятниц\w*|жұма\w*', 4), (r'суббот\w*|сенбі\w*', 5),
            (r'воскресень\w*|жексенбі\w*', 6)]
NUMBERS = {'один': 1, 'два': 2, 'две': 2, 'три': 3, 'четыре': 4, 'пять': 5,
           'бір': 1, 'екі': 2, 'үш': 3, 'төрт': 4, 'бес': 5}
def normalize_deadline(text: str | None, ref: date) -> str | None:
    if not text:
        return None
    t = text.lower()
    if re.search(r'кон(ца|ец)\s+(недел|месяц)|следующ\w*\s+(недел|месяц)|апта\w*|айдың\s+соң|келесі\s+ай', t):
        return None
    try:
        ...
        return None
    for word, days in [('послезавтра', 2), ('бүрсігүні', 2), ('завтра', 1), ('ертең', 1), ('сегодня', 0), ('бүгін', 0)]:
    ...
    matches = [(m.start(), day) for pat, day in WEEKDAYS for m in re.finditer(r'\b(?:' + pat + r')\b', t)]
    if matches:
