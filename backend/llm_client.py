    ...
    if mode == "LOCAL":
        ...
        if not local:
            raise ProcessingError(
                "LOCAL_ENDPOINT: use a numeric loopback address, e.g. 127.0.0.1"
            )
...
def chunks(segments, limit=12000):
    ...
    for s in segments:
        ...
        if n > limit:
            raise ProcessingError(
                "SEGMENT_TOO_LONG: split the transcript segment before analysis"
            )
...
def analyze(segments, meta, progress=lambda _: None):
    ...
    system = (
        "You extract meeting minutes in the source language (Russian/Kazakh/mixed). "
        ...
        "Keep existing IDs. Summary must be grounded in the transcript. All output needs human review. Schema: "
        + json.dumps(schema)
    )
    headers = (
        {"Authorization": "Bearer " + config.LLM_API_KEY}
        if meta.mode == "HYBRID" and config.LLM_API_KEY
        else {}
    )
        ...
        for i, batch in enumerate(batches):
            ...
            for attempt in range(2):
                try:
                    ...
                    previous = {a.id: a for a in result.assignments}
                    for a in candidate.assignments:
                        if a.id in previous:
                            old_evidence = {
                                (e.segment_id, e.quote) for e in previous[a.id].evidence
                            }
                            if not old_evidence.issubset(
                        ...
                        normalized = normalize_deadline(
                            a.deadline_raw, meta.meeting_at.date()
                        )
                        a.deadline = (
            ...
            if not valid:
                raise ProcessingError(
                    "LLM_INVALID_JSON: invalid schema/evidence or truncated output after 2 attempts"
                )
    return result
